...
- fix(desktop): fallback to "$HOME/Downloads" when the default downloads folder is unavailable (@Sqbika)
...
- feat(Linux): use the white icon for the Linux tray (@GaryElshaw, @Tienisto)
...
- feat: optionally enable HTTPS (encryption) when sharing via link (@Tienisto)
- feat: use switches instead of dropdowns for settings (@forecaster-cyber)
- feat: tapping on the scan button clears found devices (@Tienisto)
- feat: text message dialog is multiline only (@Tienisto)
- feat: add an option to disable animations (@Tienisto)
- feat: add an option to not save to history (@Tienisto)
- feat: add an option to customize device model (@Tienisto)
- feat(desktop): bind the "ESC" key to go to the previous page (@RiverTwilight, @Tienisto)
...
- feat(android): sharing APKs includes a version in the file name (@Tienisto)
...
- fix(android): fix the status bar icon color after picking a media file
...
- feat: add "delete from history" button when the file could not be opened (by @TheGB0077)
- feat: close message request when copied/opened link
- feat: slightly improved transfer speed
...
- feat: selection gets cleared after the finish by default (part of send modes feature)
- feat: share to multiple recipients in parallel
- feat: add troubleshooting page
- feat: add 2 buttons to receive history: open folder + delete the history
...
- feat(ex. iOS): pressing the destination directory in the progress page will open the directory
...
- feat(windows): left click on the tray icon opens the app
...
- fix(android, ios): show an error message when saving to gallery failed
- i18n: add bn, nl, uk
...
- feat: improve error handling and add the possibility to show exact error messages for debugging
- feat: add unencrypted HTTP mode (for debugging)
- feat(android): keep file name when saving to photos
- feat(desktop): use a bigger default window size if the display is big enough
...
- fix: cannot receive files when the Chinese language is active
...
- feat(desktop): show dialog instead of the bottom sheet when adding files
- feat(windows, mac): minimize to tray
- feat(windows): launch on login
- feat: add multiline toggle to message input
- fix: show correct file count on progress page
...
- fix(windows): The app sometimes crashes on start

## 1.5.0 (2023-01-09)

- feat: quick save mode
- feat: accept requests partially
- feat: set destination directory during the accept phase
- feat: rename incoming files
- feat: keep the screen on during file transfer
- feat: tap to open the selected file before sending

## 1.4.0 (2023-01-06)

- feat: support multiple local IP addresses
- feat: detect if the message is a link and add a button to open the link
...
- feat: tap to open the received file
- feat: responsive UI
